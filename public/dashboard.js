...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchPedidos() {
        try {
            ...
            pedidosContainer.innerHTML = '<p>Não foi possível carregar os pedidos.</p>';
        }
    }
        ...
        pedidosParaExibir.forEach(pedido => {
            ...
            pedidoElement.innerHTML = `
                ...
                <div class="pedido-actions">
                    <a href="/uploads/${pedido.grupo}/${pedido.comprovantePath}" target="_blank" class="download-link">
    ...
    function filtrarPedidos() {
        ...
        const activeGroup = document.querySelector('.filter-btn.active').dataset.group;

        const pedidosFiltrados = todosOsPedidos.filter(pedido => {
            const matchesName = pedido.solicitante.toLowerCase().includes(searchTerm);
            const matchesGroup = activeGroup === 'all' || pedido.grupo === activeGroup;
